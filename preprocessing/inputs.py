...
def create_embedding_matrix(feature_columns, total_vocabulary_size, init_std=0.0001, linear=False, sparse=False, device='cpu'):
    sparse_feature_columns = list(
        filter(lambda x: isinstance(x, SparseFeat), feature_columns)) if len(feature_columns) else []
    embedding_dim = sparse_feature_columns[0].embedding_dim
    embedding_dict = nn.ModuleDict({'all': nn.Embedding(total_vocabulary_size, embedding_dim if not linear else 1)})
