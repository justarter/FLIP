...
class NCELoss(nn.Module):
    ...
    def __init__(self, config, norm_term='auto', reduction='elementwise_mean'):
        ...
        self.alias = AliasMultinomial(probs=renormed_probs, data_path=config['data_path'])

        self.noise_ratio = config['pt_neg_num']
        if norm_term == 'auto':
    ...
    def forward(self, target, *args, **kwargs):
        ...
        if self.loss_type != 'full':
            ...
            if self.loss_type == 'nce':
                ...
                # else:
                    # use the full softmax for evaluation
        ...
        target_noise_logits = torch.cat([logit_target_in_model.unsqueeze(2), logit_noise_in_model], dim=2)
        target_noise_features = torch.cat([target.unsqueeze(2), noise_samples], dim=2)
        if self.reduction == 'elementwise_mean':
            return loss.mean(), target_noise_logits, target_noise_features
        elif self.reduction == 'sum':
            return loss.sum(), target_noise_logits, target_noise_features
        else:
            return loss, target_noise_logits, target_noise_features
